...
class CuciSepatu extends Component {
    constructor(props) {
        super(props);
        this.state = {
            daftarJenisPakaian: [
                { nama: 'Sepatu Futsal', harga: 2000, jumlah: 0, icon: 'shirt' },
                { nama: 'Sepatu Pantofel', harga: 2500, jumlah: 0, icon: 'shirt' },
                { nama: 'Sepatu runing', harga: 4000, jumlah: 0, icon: 'shirt' },
            ],
            cart: [],
            total: 0,
            totalModal: false,
    ...
    saveData = async () => {
        try {
            await AsyncStorage.setItem('@transaksi', JSON.stringify(this.state.transaksi),
            );
    ...
    render() {
        return (
            <View style={{ flex: 1 }}>
                ...
                <FlatGrid
                    ...
                    renderItem={({ item, index }) => (
                        ...
                        <View style={{ backgroundColor: '#03a9f4', borderRadius: 6, elevation: 3, justifyContent: 'center', alignItems: 'center', paddingVertical: 20, }}>
                            ...
                            <Text style={{ color: '#FFFFFF' }}>Rp {item.harga.toString().replace(/\B(?=(\d{3})+(?!\d))/g, '.')}</Text>
                ...
                />
                <View style={{ backgroundColor: '#03a9f4', flexDirection: 'row', justifyContent: 'center', alignItems: 'center', }}>
                    <View style={{ flex: 1 }}>
                        <Text style={{ color: '#FFFFFF', marginLeft: 20 }}>
                            Total : <Text style={{ fontWeight: 'bold' }}>Rp {this.state.total.toString().replace(/\B(?=(\d{3})+(?!\d))/g, '.')} </Text>
                        </Text>
                    </View>
                    <TouchableOpacity style={{ backgroundColor: '#212121', paddingVertical: 20, paddingHorizontal: 20 }}
                ...
                <Modal isVisible={this.state.totalModal}>
                    <View style={{ flex: 1, backgroundColor: '#FFFFFF' }}>
                        <Text
                            style={{ textAlign: 'center', fontWeight: 'bold', fontSize: 28 }}>Total
                        </Text>
                        <FlatGrid
                            itemDimension={130}
                        ...
                        <View style={{ marginVertical: 20 }}>
                            <TextInput style={{ borderBottomWidth: 1, marginHorizontal: 20 }} value={this.state.nama}
                                placeholder="masukkan nama pelanggan"
                                onChangeText={(text) => this.setState({ nama: text })}
                            />
                            <TextInput style={{ borderBottomWidth: 1, marginHorizontal: 20, marginTop: 20 }} value={this.state.alamat}
                        ...
                        <View style={{ flexDirection: 'row' }}>
                            <TouchableOpacity style={{ backgroundColor: 'crimson', paddingVertical: 20, paddingHorizontal: 20, flex: 1, justifyContent: 'center', alignItems: 'center' }}
                            ...
                            <TouchableOpacity style={{ backgroundColor: '#212121', paddingVertical: 20, paddingHorizontal: 20, flex: 1, justifyContent: 'center', alignItems: 'center' }}
                                onPress={() => this.setState({ totalModal: false }, () => this.prosesTransaksi())
                                }>
                                <Text style={{ color: '#FFFFFF' }}>
                                    selesai
                                </Text>
